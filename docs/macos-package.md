...
使用 `.pkg` 完成首次安装和后续升级，目标机无需 Rust、Python、Homebrew 或源码。当前仅支持 **Apple Silicon、用户 `vincent`、主目录 `/Users/vincent`、与构建机相同的 macOS 主版本**。安装时须登录该用户桌面，并提供管理员授权。
...
包内首次安装配置取自本机：`~/.config/clashx-rs/config.yaml`、`Country.mmdb`，以及存在时的 `subscriptions.yaml`、`wgetcloud.origin.yaml`；启动项取自 `~/Library/LaunchAgents/com.vincent.clashx-rs.plist`。构建期间不要修改这些文件。不包含 socket、PID、日志、系统代理恢复快照或指向其它仓库的软链接。
...
1. 登录目标机 `vincent` 桌面。首次安装前关闭其它代理软件及其自动启动，避免端口和系统代理冲突。
...
代理仍以 `vincent` 身份运行，不作为 root 常驻服务。只在该用户登录后自启动。首次安装沿用包内配置的系统代理、节点和局域网监听策略；当前源机配置允许局域网无认证访问，目标机须使用适合其网络的配置。
...
安装包使用 macOS `pkgbuild` / `productbuild`。系统安装器执行的 root 包装脚本仅做环境检查和临时文件调度，再通过 `launchctl asuser` / `sudo -u vincent` 运行用户级安装事务。文件替换、服务控制、配置读取均在目标用户身份下完成。事务自行管理备份和回滚，不能依赖系统安装器自动撤销脚本修改。
